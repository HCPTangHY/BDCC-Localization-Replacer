...
def replace_translation(
    ...
    translation_path: Union[Path, str],
    ...
    for file in translation_path.glob("**/*.json"):
        source_file = source_path.joinpath(
            file.relative_to(translation_path)
        ).with_suffix("")
        ...
        if source_file.suffix == ".gd":
            ...
            for item in trans_data:
                ...
                if is_replacable(item):
                    ...
                    to_replace = "".join(tscn_code[start:end])
                    new_tscn_code.append(to_replace.replace(original, translation))
                else:
            ...
            if source_file.stem.startswith("GameUI"):
                ...
                new_tscn_code = re.sub(
                    BOLD_ITALIC_RE, rf"\g<1>{bold_font.group(1)}", new_tscn_code
                )
                new_tscn_code = re.sub(
                    REGULAR_ITALIC_RE, rf"\g<1>{bold_font.group(1)}", new_tscn_code
                )
